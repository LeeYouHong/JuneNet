...
bool CSendQueue::EnQueue(void *pData, int size)
{
	...
	while (remainSize > 0)
	{
		...
		if (remainSize <= freeSize)   // 空闲内存足够
		{
			memcpy(pBuffer->GetFreeData(), (char*)pData + (size - remainSize), remainSize);
		...
		{
			// 最小数据块必须是MIN_BLOCK_SIZE
			if (pBuffer->m_Buffer.max_size() - pBuffer->m_nStartPos >= MIN_BLOCK_SIZE)
			{
				memcpy(pBuffer->GetFreeData(), (char*)pData + (size - remainSize), freeSize);
